...
import { PhaserDeck } from './phaser-deck';
import { Deck, DeckFactory, GAME_TYPE } from '../../mechanics/card/deck.manager';
import Phaser from 'phaser';

/**
 * Example usage of PhaserDeck in a Phaser scene
 */
export class PhaserDeckUIExamples {
  
  /**
   * Example 1: Standard Playing Card Deck in Phaser Scene
   */
  static createStandardDeck(scene: Phaser.Scene, x: number, y: number): PhaserDeck {
    ...
    const deckUI = new PhaserDeck(scene, x, y, deck, config);
   ...
   */
  static createUnoDeck(scene: Phaser.Scene, x: number, y: number): PhaserDeck {
   ...
   */
  static createDiceDisplay(scene: Phaser.Scene, x: number, y: number): PhaserDeck {
    ...
    const diceUI = new PhaserDeck(scene, x, y, dice, config);
  ...
  static createGameTableSetup(scene: Phaser.Scene): {
    playerDeck: PhaserDeck,
    discardPile: PhaserDeck,
    enemyDeck: PhaserDeck
  } {
    // Player deck (bottom of screen)
    const playerDeck = PhaserDeck.createPokerDeck(
      scene, 
      100, 
    ...
    // Discard pile (center)
    const discardDeck = new Deck("Discard Pile", GAME_TYPE.POKER);
    const discardPile = PhaserDeck.createDiscardPile(
      scene,
    ...
    const enemyDeck = PhaserDeck.createPokerDeck(
      scene,
    ...
    return { playerDeck, discardPile, enemyDeck };
  }
  
  /**
   * Example 5: Animated Demo Scene
   */
  static createAnimatedDemo(scene: Phaser.Scene): PhaserDeck {
    const deck = DeckFactory.createStandardDeck();
    ...
    const deckUI = new PhaserDeck(scene, 
      scene.cameras.main.width / 2 - 40, 
      scene.cameras.main.height / 2 - 60, 
      deck, 
      config
 ...
 */
export function createQuickPhaserDeckUI(
  scene: Phaser.Scene,
  x: number,
  y: number,
  ...
  return new PhaserDeck(scene, x, y, deck, config);
}
